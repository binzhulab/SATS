...
### Installation
To install from [CRAN](https://cran.r-project.org/web/packages/SATS/index.html), use the function call:
```r
...
```
Alternatively, download the package and follow the steps below. Download SATS_1.0.7.tar.gz (for Unix) or SATS_1.0.7.zip (for Windows, R version >= 4.1). To install SATS on Unix, enter the command from a Unix prompt:
```
R CMD INSTALL SATS_1.0.7.tar.gz -l path_to_install_package
```
Alternatively, SATS_1.0.7.tar.gz (for Unix) or SATS_1.0.7.zip (for Windows, R version >= 4.1) from the [Github page](https://github.com/binzhulab/SATS) are available and one may use the following commands:
```
install.packages("./SATS_1.0.7.tar.gz", repos = NULL, type = "source")
install.packages("./SATS_1.0.7.zip", repos = NULL, type = "win.binary")
```
...
- Since the $\mathbf{L}$ matrix contains the panel context associated with each patient, we first construct reference panel context using the `Panel_Context_generation()` function. The $\mathbf{L}$ matrix is then generated from the reference panel context by matching SEQ_ASSAY_ID with Patient_ID using the `L_matrix_generation()` function.
...
- To create the $\mathbf{L}$ matrix, it can be called `L_matrix_generation(Panel_context, Patient_Info)` where `Panel_context` is the panel context matrix generated by `Panel_Context_generation()` above and the `Patient_Info` contains patients ID (which will be column names of the $\mathbf{L}$ matrix) associated with `SEQ_ASSAY_ID`.
  ```r
