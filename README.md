...
```
Alternatively, download the package and follow the steps below. Download SATS_1.0.4.tar.gz (for Unix) or SATS_1.0.4.zip (for Windows, R version >= 4.1). To install SATS on Unix, enter the command from a Unix prompt:
```
R CMD INSTALL SATS_1.0.4.tar.gz -l path_to_install_package
```
Alternatively, SATS_1.0.4.tar.gz (for Unix) or SATS_1.0.4.zip (for Windows, R version >= 4.1) from the [Github page](https://github.com/binzhulab/SATS) are available and one may use the following commands:
```
install.packages("./SATS_1.0.4.tar.gz", repos = NULL, type = "source")
install.packages("./SATS_1.0.4.zip", repos = NULL, type = "win.binary")
```
